...
\begin{itemize}
    ...
    \item \textbf{Semantic Understanding}: Customers often use descriptive terms (e.g., "spicy ramen") rather than exact menu names. The system should understand meaning, not just match keywords.
...
\begin{itemize}
    ...
    \item \textbf{Data Validation}: Removes non-food items (merchandise, utensils, clothing) and ensures required fields are present
...
\begin{itemize}
    ...
    \item \textbf{Type Filtering}: Filter by content type (Menu items, Store Information, Brand Information)
...
\begin{itemize}
    \item Product images (when available) displayed alongside each result
...
\begin{itemize}
    ...
    \item \textbf{User Experience Questionnaire (UEQ)}: In addition to task-based evaluation, all three evaluators completed the User Experience Questionnaire (UEQ) to assess the overall user experience of the system.\footnote{UEQ questionnaire items are available at: \url{https://github.com/RynDroste/Information_Retrieval/blob/main/User_eval/UEQS_Items.pdf}} The UEQ is a standardized instrument that measures user experience across eight key dimensions: obstructive/supportive, complicated/easy, inefficient/efficient, confusing/clear, boring/exciting, not interesting/interesting, conventional/inventive, and usual/leading edge. Each dimension is evaluated on a scale from -3 (negative) to +3 (positive), providing comprehensive insights into both pragmatic quality (usability aspects) and hedonic quality (user experience aspects) of the system.
\end{itemize}
...
The overall system performance score is calculated as the average of all three tasks across all evaluators. The comprehensive evaluation results are summarized in Table~\ref{tab:evaluation_results}.
...
\begin{figure}[H]
\centering
\includegraphics[width=0.9\textwidth]{evaluation_charts.pdf}
...
To comprehensively assess the user experience of the system beyond task performance, three evaluators completed the User Experience Questionnaire (UEQ). The UEQ is a standardized instrument that measures user experience across eight key dimensions, each evaluated on a scale from -3 (negative) to +3 (positive). This questionnaire provides insights into both pragmatic quality (usability aspects) and hedonic quality (user experience aspects) of the system. The evaluation results from all three evaluators are summarized in Table~\ref{tab:ueq_results}.
...
The UEQ evaluation results demonstrate consistently positive user experience across all eight dimensions. The system achieved outstanding performance in several key areas: ease of use (3.0), clarity (3.0), interest level (3.0), and perception as leading-edge technology (3.0). These perfect scores indicate that evaluators found the system highly intuitive and straightforward to use, with exceptionally clear and understandable interfaces. The system was also perceived as highly engaging and innovative, reflecting the advanced semantic search capabilities and modern design approach.

In terms of pragmatic quality, the system scored well in efficiency (2.3) and supportiveness (2.3), indicating that users can accomplish their tasks effectively with minimal obstacles. The hedonic quality dimensions also received strong ratings, with excitement (2.3) and inventiveness (2.3) scores showing that the system is not only functional but also enjoyable to use.

The overall average UEQ score of 2.7 (on a -3 to +3 scale) represents an excellent positive evaluation, placing the system well above the neutral point and approaching the maximum positive rating. This comprehensive assessment confirms that the system successfully combines technical excellence with excellent user experience. The UEQ results are consistent with the task-based evaluation findings, reinforcing the conclusion that the system provides a superior user experience that meets both functional requirements and user expectations.
...
Evaluation results demonstrate significant improvements in search accuracy, cross-language capabilities, and user satisfaction. The system is production-ready and can be extended to other domains (e-commerce, document search, etc.) with minimal modifications.

\subsection{Technical Achievements}

The system has achieved several key technical milestones:

\begin{itemize}
    \item \textbf{Hybrid Search Architecture}: Successfully integrated keyword and semantic search
    \item \textbf{Cross-Language Support}: Enabled true multilingual search using LaBSE
    \item \textbf{Intelligent Ranking}: Dynamic Iight adjustment based on query type
    \item \textbf{Performance}: Maintained fast response times despite added complexity
\end{itemize}
